...
export function assertGameConfig(env) {
  ...
  if (!isUuid(env.GAME_ID)) throw new Error('WINK_GAME_ID_INVALID');
...
export async function verifyGameConfig({ env = process.env, rootDir = process.cwd() } = {}) {
  const plan = assertGameConfig(env);
  const config = JSON.parse(
    await fs.readFile(path.join(rootDir, 'public', 'wink-runtime-config.json'), 'utf8'),
  );
  if (
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  try {
    ...
    process.stderr.write(`${error instanceof Error ? error.message : 'WINK_GAME_CONFIG_INVALID'}\n`);
    process.exitCode = 2;
  }
}
